 ...
 * Enhanced globe: 50 iconic cities, live Geoapify Places, country-grouped sidebar,
 * redesigned mode selector, animated Surprise Me, real Earth texture.
 */
...
/* ── Country flags ─────────────────────────────────────────────── */
const COUNTRY_FLAGS = {
  Japan:'🇯🇵',China:'🇨🇳',Singapore:'🇸🇬',Thailand:'🇹🇭',UAE:'🇦🇪',India:'🇮🇳',
  Turkey:'🇹🇷','South Korea':'🇰🇷',Indonesia:'🇮🇩',Nepal:'🇳🇵',Vietnam:'🇻🇳',
  France:'🇫🇷',UK:'🇬🇧',Italy:'🇮🇹',Spain:'🇪🇸',Netherlands:'🇳🇱',Czechia:'🇨🇿',
  Austria:'🇦🇹',Germany:'🇩🇪',Greece:'🇬🇷',Portugal:'🇵🇹',Iceland:'🇮🇸',
  USA:'🇺🇸',Canada:'🇨🇦',Mexico:'🇲🇽',Argentina:'🇦🇷',Brazil:'🇧🇷',Peru:'🇵🇪',
  Colombia:'🇨🇴',Cuba:'🇨🇺',Egypt:'🇪🇬','South Africa':'🇿🇦',Morocco:'🇲🇦',
  Kenya:'🇰🇪',Tanzania:'🇹🇿',Nigeria:'🇳🇬',Australia:'🇦🇺','New Zealand':'🇳🇿',
  Israel:'🇮🇱',Qatar:'🇶🇦',Oman:'🇴🇲',Lebanon:'🇱🇧',Jordan:'🇯🇴',
};
function countryFlag(country) { return COUNTRY_FLAGS[country] || '🌐'; }

/* ── City data (50 iconic cities) ──────────────────────────────── */
const CITIES = [
  // Asia
...
const CONTINENT_ORDER = ['Asia','Europe','Americas','Africa','Oceania','Middle East'];

const MODES = [
  { id:'discover', icon:'🌍', label:'Discover', desc:'Explore by region',        swatchType:'none' },
  { id:'budget',   icon:'💰', label:'Budget',   desc:'Color by cost level',      swatchType:'gradient', swatch:'linear-gradient(90deg,#10B981,#F59E0B,#EF4444)' },
  { id:'climate',  icon:'🌤️', label:'Climate',  desc:'Color by climate type',    swatchType:'dots' },
  { id:'routes',   icon:'✈️', label:'Routes',   desc:'See flight connections',   swatchType:'line' },
];
const MODE_LEGENDS = {
...
export default function WorldPage() {
  ...
  const [flyingToast,       setFlyingToast]       = useState(null);
  ...
  useEffect(() => {
    ...
    // Use the browser's language (e.g. 'en', 'fr') so place names appear in the
    // user's locale. Fall back to 'en' so we never get undefined.
    const userLang = (typeof navigator !== 'undefined' ? navigator.language : 'en')
      .split('-')[0] || 'en';
    const get = (cats,lim) =>
      fetch(`/api/geoapify/places?categories=${encodeURIComponent(cats)}&lat=${lat}&lng=${lng}&radius=10000&limit=${lim}&lang=${userLang}`)
        .then(r=>r.json())
        .then(d=>({ results: d.results ?? [], available: d.available !== false }))
        .catch(()=>({ results: [], available: true }));
    Promise.all([
      // tourism.sights + entertainment.museum + tourism.attraction are all valid
      // Geoapify v2 categories. 'entertainment.attraction' was invalid and returned
      // empty results — replaced with 'tourism.attraction'.
      get('tourism.sights,entertainment.museum,tourism.attraction',6),
      get('catering.restaurant,catering.cafe,catering.bar',6),
      get('accommodation.hotel,accommodation.hostel',4),
    ]).then(([a,b,c]) => {
      setPlaces({ highlights: a.results, food: b.results, stay: c.results });
      setPlacesApiAvailable(a.available && b.available && c.available);
      setLoadingPlaces(false);
    });
  }, [selectedCity]);

  /* Direct globe marker click — immediate (no delay) */
  const handleMarkerClick = useCallback((marker,idx) => {
    globeApiRef.current?.flyTo(marker.lat, marker.lng);
  ...
  /* flyTo — used by sidebar list + Surprise Me — shows toast, opens panel after 800ms */
  const flyTo = useCallback((city,idx) => {
    clearTimeout(flyTimerRef.current);
    globeApiRef.current?.flyTo(city.lat, city.lng);
    setSelectedCity(null);    // keep autoSpin=true while globe rotates
    setSelectedIdx(-1);
    setFlyingToast(city.label);
  ...
  /* Filter cities for search/continent */
  const isSearching = search.trim().length > 0;
  const filteredCities = useMemo(() => {
    if (isSearching) return CITIES.filter(c => c.label.toLowerCase().includes(search.toLowerCase()) || c.country.toLowerCase().includes(search.toLowerCase()));
    return activeContinent==='All' ? CITIES : CITIES.filter(c=>c.continent===activeContinent);
  }, [search, activeContinent, isSearching]);
  ...
  /* Grouped cities for browse mode */
  const groupedCities = useMemo(() => {
    if (isSearching) return [];
    const result = [];
    const continentsToDo = activeContinent === 'All' ? CONTINENT_ORDER : [activeContinent];
    continentsToDo.forEach(cont => {
      const inCont = filteredCities.filter(c => c.continent === cont);
      if (inCont.length === 0) return;
      if (activeContinent === 'All') {
        result.push({ key:`cont-${cont}`, isContHeader:true, label:cont, cities:[] });
      }
      const byCountry = {};
      inCont.forEach(city => { (byCountry[city.country] ??= []).push(city); });
      Object.entries(byCountry).forEach(([country, cities]) => {
        result.push({ key:`${cont}-${country}`, isContHeader:false, continent:cont, country, cities });
      });
    });
    return result;
  }, [filteredCities, activeContinent, isSearching]);
    ...
    <>
      ...
      <div className="fixed inset-0 top-16 flex overflow-hidden bg-atlas-bg">
        ...
        <div className={`flex-shrink-0 transition-all duration-300 flex flex-col overflow-hidden ${sidebarOpen?'w-72':'w-0'}`}
          ...
          <div className="flex flex-col h-full overflow-hidden p-4 gap-3">
            ...
            <div className="flex flex-wrap gap-1.5 flex-shrink-0">
              {CONTINENTS.map(c=>(
                <button key={c} onClick={()=>{setActiveContinent(c);setSearch('');setCollapsedCountries({});}}
            ...
            {/* City list — grouped by country, or flat when searching */}
                  ...
                  {filteredCities.map(city=>{
                    ...
                    return (
                      <button key={`${city.label}-${city.country}`} onClick={()=>flyTo(city,idx)}
                        ...
                        <span className="text-[10px] text-atlas-text-muted flex-shrink-0">{city.country}</span>
                      </button>
                    );
                  })}
                  {filteredCities.length===0 && <p className="text-center text-[12px] text-atlas-text-muted py-4">No cities found</p>}
                </div>
              ) : (
                <div>
                  {groupedCities.map(group=>{
                    if (group.isContHeader) {
                      return (
                        <div key={group.key} className="px-3 pt-4 pb-1.5 first:pt-1">
                          <span className="text-[10px] font-black text-atlas-text-muted uppercase tracking-widest">{group.label}</span>
                        </div>
                      );
                    }
                    const isCollapsed = collapsedCountries[group.key];
                    return (
                      <div key={group.key}>
                        <button
                          onClick={()=>setCollapsedCountries(prev=>({...prev,[group.key]:!prev[group.key]}))}
                          className="w-full flex items-center gap-2 px-3 py-1.5 hover:bg-white/[0.04] rounded-lg transition-colors group">
                          <span className="text-sm leading-none">{countryFlag(group.country)}</span>
                          <span className="flex-1 text-xs font-semibold text-atlas-text-secondary group-hover:text-white truncate text-left">{group.country}</span>
                          <span className="text-[10px] text-atlas-text-muted bg-white/[0.06] rounded-full px-1.5 py-0.5 flex-shrink-0">{group.cities.length}</span>
                          <svg className={`w-3 h-3 text-atlas-text-muted transition-transform flex-shrink-0 ${isCollapsed?'':'rotate-180'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        {!isCollapsed && group.cities.map(city=>{
                          ...
                          return (
                            <button key={`${city.label}-${city.country}`} onClick={()=>flyTo(city,idx)}
                              className={`w-full text-left pl-9 pr-3 py-1.5 rounded-xl transition-all text-sm flex items-center gap-2 ${isSel?'bg-atlas-blue/20 border border-atlas-blue/30 text-white':'hover:bg-white/[0.04] text-atlas-text-secondary hover:text-white'}`}>
                              <span className="w-2 h-2 rounded-full flex-shrink-0" style={{background:city.color}}/>
                              <span className="flex-1 font-medium truncate">{city.label}</span>
                            </button>
                          );
        ...
        <div className="relative flex-1 overflow-hidden">
          ...
          {/* Hint */}
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 glass px-3 py-1.5 rounded-full text-xs text-atlas-text-secondary border border-white/[0.07] pointer-events-none whitespace-nowrap">
            <span className="text-atlas-cyan font-bold">{CITIES.length}</span> cities · hover to pause · click to explore
          </div>

          {/* Flying toast */}
          {flyingToast && (
            <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 glass px-4 py-2 rounded-full text-sm text-white border border-atlas-blue/40 pointer-events-none flex items-center gap-2"
              style={{backdropFilter:'blur(16px)',animation:'slide-up 0.2s both'}}>
              <span className="animate-pulse">✈</span>
              <span>Flying to <strong>{flyingToast}</strong>…</span>
            </div>
          )}

          {/* Mode selector — top right floating panel */}
          <div className="absolute top-4 right-4 z-30">
            {modesPanelOpen ? (
              <div className="glass rounded-2xl p-3 border border-white/10 w-52"
                style={{backdropFilter:'blur(24px)',background:'rgba(5,8,16,0.92)'}}>
                <div className="flex items-center justify-between mb-2.5 px-0.5">
                  <span className="text-[10px] font-bold text-atlas-text-muted uppercase tracking-wider">View Mode</span>
                  <button onClick={()=>setModesPanelOpen(false)} className="text-atlas-text-muted hover:text-white text-xs w-5 h-5 flex items-center justify-center rounded hover:bg-white/10">✕</button>
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  {MODES.map(m=>(
                    <button key={m.id} onClick={()=>{setMode(m.id);}}
                      className={`flex flex-col items-start gap-1 p-2.5 rounded-xl transition-all text-left ${mode===m.id?'bg-white/10 border border-atlas-blue/60':'hover:bg-white/[0.06] border border-transparent'}`}>
                      <span className="text-xl leading-none">{m.icon}</span>
                      <span className="text-xs font-semibold text-white">{m.label}</span>
                      <span className="text-[10px] text-atlas-text-muted leading-tight">{m.desc}</span>
                      {m.swatchType==='gradient' && <div className="w-full h-1.5 rounded-full mt-0.5" style={{background:m.swatch}}/>}
                      {m.swatchType==='dots' && (
                        <div className="flex gap-0.5 mt-0.5">
                          {['#f97316','#3B82F6','#eab308','#8B5CF6','#10B981'].map(c=>(
                            <span key={c} className="w-2 h-2 rounded-full" style={{background:c}}/>
                          ))}
                        </div>
                      )}
                      {m.swatchType==='line' && <div className="w-full h-0.5 rounded mt-0.5 bg-atlas-cyan opacity-70"/>}
                    </button>
                  ))}
                </div>
              </div>
            ) : (
              <button onClick={()=>setModesPanelOpen(true)}
                className="glass w-10 h-10 rounded-xl flex items-center justify-center text-xl hover:bg-white/10 border border-white/10 transition-all"
                title={`Mode: ${MODES.find(m=>m.id===mode)?.label}`}>
          ...
          <div className="flex-shrink-0 w-80 flex flex-col overflow-hidden"
            ...
            <div className="relative p-5 flex-shrink-0" style={{background:`linear-gradient(135deg, ${selectedCity.color}22, transparent 60%)`}}>
              ...
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center text-xl"
                  style={{background:`${selectedCity.color}30`,border:`1px solid ${selectedCity.color}40`}}>
                  {countryFlag(selectedCity.country)}
                </div>
                <div>
            ...
            <div className="px-4 py-2 flex-shrink-0 border-b border-white/[0.06]">
              <div className="flex gap-1">
                {[{id:'highlights',label:'⭐ Sights'},{id:'food',label:'🍜 Food'},{id:'stay',label:'🏨 Stay'}].map(tab=>(
                  <button key={tab.id} onClick={()=>setDetailTab(tab.id)}
                    className={`flex-1 py-1.5 text-xs font-medium rounded-lg transition-all ${detailTab===tab.id?'bg-atlas-blue/20 text-atlas-cyan border border-atlas-blue/30':'text-atlas-text-muted hover:text-white'}`}>
                    {tab.label}
                  </button>
                ))}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 min-h-0">
              {loadingPlaces
                ...
                : tabPlaces.length>0
                  ...
                  : <div className="text-center py-8"><div className="text-3xl mb-2">🔍</div><p className="text-atlas-text-muted text-sm">{placesApiAvailable ? 'No places in this area' : 'No places found'}</p>{!placesApiAvailable && <p className="text-atlas-text-muted text-xs mt-1">Add GEOAPIFY_API_KEY to .env.local and restart the dev server.</p>}</div>
              }
            </div>
